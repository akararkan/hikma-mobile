...
/* Kept deliberately small and system-safe: a reel is watched on someone else's
   ...
   APPEND-ONLY, both tables. Ids 0..2 (and colours 0..7) are already in
   published overlay.json documents, and idx() on an older client clamps an
   index it does not know to 0 — so a new face degrades to Sans and a new
   colour to white on last month's build, and nothing breaks. Reordering,
   though, would silently restyle every published reel. The `italic` flag is
   data for the css side; native carries italics in the face name. */
export const FONTS = [
  ...
  { id: 3, label: 'Light',  css: 'var(--sans, system-ui, sans-serif)', weight: 400 },
  { id: 4, label: 'Italic', css: 'var(--serif, Georgia, serif)',       weight: 600, italic: 1 },
  { id: 5, label: 'Vazir',  css: 'Vazirmatn, var(--sans, system-ui, sans-serif)', weight: 700 },
]
export const COLORS = [
  '#ffffff', '#0b131d', '#ffd166', '#e07a72', '#b9d6f2', '#7bd389', '#c792ea', '#ff8fab',
  /* Second course, 2026-08-23: the brand pair first, then story-friendly
     strongs. All appended — see the table rule above. */
  '#002147', '#49B6FF', '#e63946', '#2a9d8f', '#f4a261', '#8338ec', '#606c38', '#ffe8d6',
]
export const ALIGNS = ['start', 'center', 'end']

/* One luminance rule for every surface that has to plate a colour, replacing
   the old "the palette has exactly one dark entry" binary — that stopped
   being true the day the second course landed. Plain sRGB weights on the raw
   channels: this classifies a small closed palette, it does not grade film. */
export function colorIsDark(hex) {
...
/** The plate behind an item's glyphs, by bg mode: 1 = solid pill, 2 = soft
 *  wash. Always the OPPOSITE pole of the ink, so navy on a plate reads as
 *  well as white on one. */
export function plateColor(colorIndex, bgMode) {
  const dark = colorIsDark(COLORS[colorIndex] ?? COLORS[0])
  if (bgMode === 2) return dark ? 'rgba(255,255,255,0.6)' : 'rgba(0,0,0,0.26)'
  return dark ? 'rgba(255,255,255,0.92)' : 'rgba(0,0,0,0.46)'
}
...
export function cleanItem(raw) {
  ...
  return {
    ...
    a: idx(raw.a, ALIGNS),
    m: idx(raw.m, MOTIONS),
    /* Three plate modes since 2026-08-23: 0 none (shadowed glyphs), 1 solid
       pill, 2 soft wash. An older client's `raw.bg ? 1 : 0` reads a 2 as the
       solid pill — a plate either way, so the words stay readable. */
    bg: Number(raw.bg) === 2 ? 2 : raw.bg ? 1 : 0,
  }
}
