...
export const users = {
  ...
  /* §11.3 / §11.4 — `signal` rides beside the {page,size} query so paged
     screens can cancel a superseded read; split off so it never hits the wire. */
  async followers(id, { signal, ...q } = {}) { return page(await http.get(`/api/v1/users/${id}/followers`, q, { signal }), userFrom) },
  async following(id, { signal, ...q } = {}) { return page(await http.get(`/api/v1/users/${id}/following`, q, { signal }), userFrom) },
