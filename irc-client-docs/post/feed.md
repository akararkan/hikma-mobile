...
| `mediaUrl` | string | Cover media, or `null`. When the leading media is a **VIDEO** and the server generated a poster frame at upload time, this is that poster **image** (JPEG); otherwise the first media URL as before. Paintable as an `<img>` whenever it carries an image extension |
