...
export function adminFrom(dto) {
  if (!dto) return null
  /* ChannelService loads these rows through `findActiveByIdIn`, so a deleted
     admin arrives with a userId and no name at all — the same conclusive
     shape the chat roster produces (adapters.js §authorFrom). */
...
export const channels = {
  ...
  invites: {
    ...
    async redeem(token) {
      ...
      const status = res?.status || (res?.conversation ? 'JOINED' : 'PENDING_APPROVAL')
      return {
        status,
