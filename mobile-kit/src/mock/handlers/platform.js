 ...
 *  2FA or verifying the email in the demo really does move the score.
 *  `recovery` and `email_verified` deliberately read ONE flag, exactly like
 *  the backend (email IS the recovery channel): 50 points flip together. */
...
function scoreDto(db) {
  ...
  const emailOk = !!meRowOf(db)?.isEmailVerified
  const LIVE = { two_factor: twoFa, recovery: emailOk, email_verified: emailOk }
  const items = arr(db.safety?.score?.items).map(i => {
    const passed = i.key in LIVE ? LIVE[i.key] : !!i.passed
...
/** The signed-in user's LIVE users-row. users.js re-points db.me at it on the
 *  first /users/me read, but resolve by id anyway — these routes can run
 *  before any users read, and writes must land on the row userResponse
 *  serializes (isEmailVerified feeds /users/me AND the checkup score). */
const meRowOf = (db) => userById(db, idOf(db.me)) || db.me || null
...
export const routes = [
  ...
  /* ---------------- security: history, step-up, phone, email ---------------- */
  {
  ...
  /* Email verification — the two-endpoint mirror of the phone flow, minus the
     destination: the code can only go to the address already on the account.
     Any 6-digit code verifies (like the phone mock); the write lands on the
     LIVE users-row so /users/me flips isEmailVerified and the checkup score
     moves by 50 points on the next read. */
  {
    m: 'POST', p: /^\/api\/v1\/security\/email\/request$/,
    fn: (db) => {
      const me = meRowOf(db)
      if (!me?.email) throw mockError(400, 'EMAIL_MISSING', 'There is no email address on this account to verify.')
      if (me.isEmailVerified) throw mockError(409, 'EMAIL_ALREADY_VERIFIED', 'Your email address is already verified.')
      securityOf(db).emailVerify = { requested: true }
      return NO_CONTENT                                          // 202, empty body — the code "arrives" by email
    },
  },
  {
    m: 'POST', p: /^\/api\/v1\/security\/email\/verify$/,
    fn: (db, { body }) => {
      const me = meRowOf(db)
      if (me?.isEmailVerified) throw mockError(409, 'EMAIL_ALREADY_VERIFIED', 'Your email address is already verified.')
      if (!/^\d{6}$/.test(String(body?.code || ''))) throw mockError(400, 'OTP_INVALID', 'Invalid or expired verification code.')
      if (me) me.isEmailVerified = true
      if (db.me && db.me !== me) db.me.isEmailVerified = true    // keep the fixture's own `me` fragment coherent
      return { verified: true, email: me?.email || null }
    },
  },
  {
