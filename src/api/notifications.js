...
/** Server href grammar → this client's route, or null when there is no route
 *  to give. The ONE home for the mapping: the inbox rows below and the
 *  push-tap handler (lib/pushNotify.ts) both go through here — a href that
 *  routes from the inbox but 404s from a lock-screen tap of the very same
 *  notification reads as a random bug. */
export function clientHrefOf(href) {
  if (typeof href !== 'string' || !href.startsWith('/')) return null
  if (UNROUTABLE.test(href)) return null
  for (const [re, to] of LINK_REWRITE) if (re.test(href)) return href.replace(re, to)
  return href
}
...
export const notifications = {
  ...
  async list({ category, type, unread, page = 0, size = 30, signal } = {}) {
  ...
  async unreadList({ page = 0, size = 20, signal } = {}) {
    return pageOf(await http.get('/api/v1/notifications/unread', { page, size }, { signal }), { page, size })
  },
  ...
  /** ≤ 200 DISTINCT ids (dedupe before capping, or duplicates waste cap
   *  slots); foreign/unknown ids are silently skipped. → { updated } */
  async markReadBulk(ids) {
    const r = await http.patch('/api/v1/notifications/read', { ids: [...new Set(ids || [])].slice(0, 200) })
    return r?.updated ?? 0
  },
...
function sharedConnect() {
  ...
  /* The SSE payload is the RAW persisted row, not the REST DTO (realtime.md):
     the upsert key is `notificationId` (not `id`), there is no `isRead` /
     `category` / actor display fields, and a coalesced bump means the row is
     UNREAD AGAIN. So: alias the id before adapting, pin unread, and carry
     `coalesced` + `groupKey` through — a coalesced payload omits title /
     actorId / createdAt, so consumers merge it into the existing row (or
     refetch for the enriched shape) rather than replacing wholesale. */
  es.addEventListener('notification', (e) => {
    beat()
    const d = parse(e)
    fan('onNotification', {
      ...notifFrom({ ...d, id: d.notificationId }),
      unread: true,
      coalesced: d.coalesced === true,
