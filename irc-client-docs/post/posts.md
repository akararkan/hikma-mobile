...
| `thumbnailUrl` | string | Server-generated poster frame (JPEG, ~720px wide) for the post's **first VIDEO** media — paint it while the player attaches. `null` for image-only posts, for videos uploaded before poster generation existed, and when the server's ffmpeg was unavailable at upload time (extraction is best-effort and never fails the upload) |
...
For the **first VIDEO part** the server additionally extracts a poster frame
(ffmpeg, ~0.4s in, scaled to ≤720px wide, JPEG) and stores it beside the video under
`posts/media/`; its URL is persisted on the post and returned as
`PostResponse.thumbnailUrl`. On feed/reels list rows the same image becomes the
`mediaUrl` cover slot. Extraction is best-effort: if ffmpeg is missing or the clip is
unreadable, the post is created normally with `thumbnailUrl: null`.
