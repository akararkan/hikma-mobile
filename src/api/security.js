/* =========================================================
   ...
   history, step-up re-auth, phone binding, email
   verification, public OTP.
...
export const OTP_PURPOSES = ['LOGIN', 'PHONE_VERIFY', 'PHONE_CHANGE', 'EMAIL_VERIFY', 'EMAIL_CHANGE', 'PASSWORD_RESET', 'STEP_UP']

export const security = {
  /* ---- sessions (Active Sessions = refresh_tokens) ----
     The login path now persists the whole display row — sid, deviceName,
     platform, ip, lastSeenAt — so new sessions arrive fully populated. The
     additive columns are nullable though: rows minted BEFORE the sessions
     migration can still carry a null sid, so don't key rows on sid alone and
     don't offer the two sid-addressed actions when it is missing. */
  sessions: {
  ...
  async loginHistory({ signal, ...q } = {}) {
    return page(await http.get('/api/v1/security/login-history', q, { signal }))   // [{ip,userAgent,method,outcome,ts}]
  },

  /* ---- step-up ---- */
  /** Arm the step-up window with a fresh password OR a TOTP code.
   *  Success → 204. Wrong password → 400 STEP_UP_BAD_PASSWORD; wrong code →
   *  400 TWO_FA_INVALID; neither supplied → 400 STEP_UP_REQUIRED. All three
   *  arrive in the standard error envelope now (the old bare-400 escape for
   *  code/empty-body was fixed server-side), so errorText(e) always works. */
  ...
  /* ---- email verification (logged-in) ----
     ...
     isEmailVerified, so the Settings card survives a reload.
     The code is 6 digits and lives 15 MINUTES (not the phone flow's 5);
     budgets: 3 sends per account/hour, 10 per IP/hour, 5 wrong codes burn
     the challenge. */
  email: {
    request() { return http.post('/api/v1/security/email/request') },              // 202; 409 EMAIL_ALREADY_VERIFIED; 400 EMAIL_MISSING; 429
    verify(code) { return http.post('/api/v1/security/email/verify', { code }) },  // {verified:true, email}; 400 OTP_INVALID; 409 EMAIL_ALREADY_VERIFIED
  },
