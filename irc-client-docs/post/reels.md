...
asset, even when `mediaUrl` points at a cover thumbnail. `mediaUrl` is the cover slot:
for reels uploaded since server-side poster generation it is the ffmpeg-extracted
poster JPEG (`PostResponse.thumbnailUrl`); for older video reels it is still the video
file itself, so keep the extension check before painting it as an image. Rows whose canonical post has
