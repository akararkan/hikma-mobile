...
/* A deleted account keeps its rows. Its messages, posts and comments stay
   where they were and are attributed to it rather than removed — the backend
   says so outright (settings/data-export-deletion.md §16) and every major
   platform does the same. What the client cannot do is render it as a live
   member, and until this it did: `full` fell through to 'Member' and `handle`
   to '@member', which is what a real account with a failed hydration looks
   like.

   The account arrives in TWO shapes, because the two hydration paths differ:

     · Chat hydrates senders, rosters and requesters through
       `findActiveByIdIn`, which filters `deletedAt IS NULL` — so a deleted
       user comes back with BOTH name fields null and only the id left. The
       mappers that know that contract pass `deleted: true`; nothing else in
       those DTOs can produce that shape.
     · Posts read the user row directly (`PostHydrator.bulkLoadAuthors` →
       `findAllById`), so they get the row the nightly purge rewrote:
       `deleted_user_<12 hex>` / "Deleted User"
       (`AccountLifecycleService.anonymizeAndPurge`). That username is
       machine-generated and unique, so matching it is exact rather than a
       guess about someone's chosen name.

   The purge does NOT clear `profileImage`, so the avatar of a purged account
   still comes down the wire on their old posts. Dropping it here is the only
   place that can refuse it. */
const ANONYMIZED_USERNAME = /^deleted_user_[0-9a-f]{12}$/i

/** Is this author stub a deleted account? `deleted: true` is the explicit
 *  form, for the DTOs whose contract makes namelessness conclusive. */
export function isDeletedAuthor(a) {
  if (!a) return false
  if (a.deleted === true) return true
  return ANONYMIZED_USERNAME.test(String(a.username || a.authorUsername || ''))
}

/** What every surface calls a deleted account. One string, so the inbox, a
 *  group bubble, a post byline and a seen-by row cannot disagree. */
export const DELETED_ACCOUNT_LABEL = 'Deleted account'

export function authorFrom(a, fallbackId) {
  const id = a?.id || fallbackId || ''
  const gone = isDeletedAuthor(a)
  const full = gone
    ? DELETED_ACCOUNT_LABEL
    : (a?.fullName || a?.authorFullName || a?.username || a?.authorUsername || 'Member')
  /* Empty, not 'member'. Every call site that offers a tap-through or an
     @-line already tests the handle for truthiness, so blanking it is what
     removes both — and the tap would have gone to a profile that 404s. */
  const handle = gone ? '' : handleOf(a?.username || a?.authorUsername)
  return {
    id,
    full,
    handle,
    /* initialsOf('') is the neutral '··'; 'DA' would read as someone's name. */
    initials: gone ? initialsOf('') : initialsOf(full),
    avc: gradientFor(id || handle),
    profileImage: gone ? null : assetUrl(a?.profileImage || a?.authorProfileImage || null),
    verified: gone ? false : !!a?.verified,
    role: a?.role || 'MEMBER',
    deleted: gone,
  }
}
...
export function meFrom(u) {
  ...
  return {
    ...
    /* Own record only in practice (public cards don't carry it) — the Settings
       verify-email card shows which inbox the code went to. */
    email: u.email || '',
    ...
    /* NOT on the followers/following wire — UserResponse list rows carry no
       relationship flag, so this is false there and must never drive a Follow
       button. Read GET /users/{id}/social-status (useFollowState) instead;
       kept only so a future wire that does send it flows through. */
    isFollowing: !!(u.isFollowing ?? p.isFollowing),
    posts: p.postCount ?? 0,
...
/* Built once at module load: every `toLocale*` call with an options bag
   ...
   pattern resolution, and timeAgo runs at adapter parse time for every row. */
const DAY_FMT = new Intl.DateTimeFormat(undefined, { day: 'numeric', month: 'short' })
...
/** PostMediaDto.variants → absolutised client map. The values are urls and
 *  arrive relative like every other media url on the wire; empty/legacy → null
 *  so renderers can gate on the map's existence. */
function variantsFrom(m) {
  const raw = m?.variants
  if (!raw) return null
  /** @type {Record<string, string>} */
  const out = {}
  for (const k of Object.keys(raw)) { if (raw[k]) out[k] = assetUrl(raw[k]) }
  return Object.keys(out).length ? out : null
}

function mediaFromUrls(urls = [], types = [], poster = null, rich = []) {
  const tarr = types || []   // default param only catches `undefined`; the backend can send `mediaTypes: null`
  /* `rich` is PostResponse.media — the per-item descriptors, index-aligned
     with mediaUrls server-side (PostHydrator.buildMedia). Additive: `variants`
     (hls / preview / the rendition ladder) and `blurhash` ride along when the
     pipeline produced them; legacy rows simply carry neither. */
  const rarr = rich || []
  /* `poster` is PostResponse.thumbnailUrl — the server-side ffmpeg frame for
     the FIRST video of the post, so it is consumed by the first VIDEO entry
     only (a second video in a carousel has no generated poster). */
  let posterLeft = poster ? assetUrl(poster) : null
  return (urls || []).map((url, i) => {
    const t = tarr[i] || 'IMAGE'
    const src = assetUrl(url)   // backend media URLs are relative → make absolute
    const extra = { variants: variantsFrom(rarr[i]), blurhash: rarr[i]?.blurhash || null }
    if (t === 'IMAGE') return { type: 'IMAGE', url: src, label: 'image', bg: `center/cover no-repeat url("${src}")`, ratio: '16/10', ...extra }
    if (t === 'VIDEO') {
      const p = posterLeft
      posterLeft = null
      return { type: 'VIDEO', url: src, poster: p, label: 'video', bg: 'linear-gradient(160deg,#1a2836,#0b131d)', ratio: '16/10', ...extra }
    }
    return { type: t, url: src, label: t.toLowerCase(), ...extra }
  })
}
...
export function postFromFeedItem(dto) {
  ...
  // Additive, REEL-only: the CMAF/fMP4 HLS master. `videoUrl` above stays the
  // progressive MP4 exactly as before — players prefer this, fall back to that.
  const videoHls = dto.videoHlsUrl ? assetUrl(dto.videoHlsUrl) : null
  ...
  if (isReel && (video || cover)) {
    ...
    media = video || !isImageUrl(cover)
      /* The feed row carries no variants map, so the HLS master is folded into
         one here — it is what lets a renderer that only sees `media` (PostMedia)
         make the same hls-first choice the reels pool makes off `videoHlsUrl`. */
      ? [{ type: 'VIDEO', url: video || cover, poster: cover, label: 'video', bg: 'linear-gradient(160deg,#1a2836,#0b131d)', ratio: '9/16', variants: videoHls ? { hls: videoHls } : null }]
...
export function postFromResponse(dto) {
  const a = authorFrom(dto.author, dto.authorId)
  /* VOICE_POST's mediaUrls are mapped like everyone else's — the recorded
     audio comes back there as an AUDIO/AUDIO_TRACK part (only a picked Sound
     uses `audioTrackUrl`), and the lift below turns it into `audioUrl`.
     Dropping it here is what made a reloaded voice post play nothing. The
     url-less placeholder survives only for rows with no media at all. */
  const all = dto.postType === 'VOICE_POST' && !(dto.mediaUrls || []).length
    ? [{ type: 'AUDIO', label: dto.audioTrackName || 'voice note', duration: '0:00' }]
    : mediaFromUrls(dto.mediaUrls, dto.mediaTypes, dto.thumbnailUrl, dto.media)
  ...
  /* Typed OTHER is the whole signal: nothing else on this platform uploads a
     ...
     broken frame. */
  /* REEL-scoped now: regular posts can carry real generic-file attachments
     (typed OTHER by the backend) since POST_MEDIA accepts FILE — only a reel
     treats an OTHER part as its overlay document. */
  const overlay = dto.postType === 'REEL' ? all.find(m => m.type === 'OTHER' && m.url) : null
  ...
  /* A RECORDED voice post uploads its audio as an ordinary file part, so it
     comes back in mediaUrls typed AUDIO with `audioTrackUrl` null — only a
     picked Sound fills audioTrackUrl. Lift it out exactly like the reel's
     voiceover: the transport plays it, and an AUDIO entry left inside `media`
     would render as a broken image tile. */
  const voiceTrack = dto.postType === 'VOICE_POST' ? all.find(m => /^AUDIO/.test(m.type) && m.url) : null
  const media = all.filter(m => m !== overlay && m !== voice && m !== voiceTrack)
  return {
    ...
    media,
    // VOICE_POST playable audio: the picked-Sound field first, else the
    // recorded upload lifted from mediaUrls above.
    audioUrl: assetUrl(dto.audioTrackUrl || null) || voiceTrack?.url || null,
...
export function commentFrom(dto) {
  ...
  return {
    ...
    edited: !!dto.edited,
    /* engagement.md §3.1 puts one inline image or clip on a comment. REST
       CommentResponse carries only mediaUrl/mediaType — the thumbnail arrives
       on SSE frames alone, so render must fall back to mediaUrl. A REPLY
       carries mediaUrl with NO mediaType, which is why the renderer treats a
       missing type as IMAGE rather than hiding the attachment. */
    mediaUrl: assetUrl(dto.mediaUrl || null),
    mediaType: dto.mediaType || null,
    mediaThumbnailUrl: assetUrl(dto.mediaThumbnailUrl || null),
    /* The raw ISO stamp IS the comments cursor (engagement.md §3.2) — dropping
       it forced every paging caller back to raw http. */
    createdAt: dto.createdAt || null,
  }
}
...
export function researchFrom(dto) {
  ...
  return {
    ...
    /* Card-level share/deep-link affordances need these without a second GET —
       both are on ResearchSummaryResponse. */
    slug: dto.slug || null,
    ...
    metrics: {
      ...
      shares: dto.shareCount || 0,   // 7th documented counter — SHARE_COUNT_UPDATED patches it
    },
...
export function researchDetailFrom(dto) {
  ...
  return {
    ...
    publishedAt: dto.publishedAt || null,                       // the type promises it; summary.createdAt already prefers it
...
export function soundFrom(dto) {
  ...
  return {
    ...
    useCount: dto.useCount ?? null,              // only /usage carries it; null = not on this wire shape
