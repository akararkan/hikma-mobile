...
const review = {
   ...
   *   empty page.
   */
  async list({ status = 'IN_REVIEW', entityType, slaBreached = false, sort = 'risk', page = 0, pageSize = 50, signal } = {}) {
...
const model = {
   ...
   */
  async trainingExamples({ source, page = 0, pageSize = 50, signal } = {}) {
    const p = paging(page, pageSize)
   ...
   *   not 50 (drift: api.md).
   */
  async versions({ page = 0, pageSize = 20, signal } = {}) {
    const res = await http.get('/api/v1/admin/moderation/model/versions', paging(page, pageSize), { signal })
...
const queue = {
   ...
   */
  async list({ source, targetType, page = 0, pageSize = 50, signal } = {}) {
