...
/* The 44 NotificationType rows, grouped for a readable matrix. ACCOUNT_WARNING
...
export const NOTIFICATION_GROUPS = [
  ...
  /* Left out of the curated groups: UNFOLLOWED, BLOCKED, UNBLOCKED, RESTRICTED,
     ...
     has no callers). Also ungrouped: SOUND_REJECTED (live — moderation sends
     it) and ADMIN_ANOMALY (live, staff recipients only). resolvedMatrix walks
     the WHOLE enum, so the response always carries them and they DO render —
     under "Other", where the panel says out loud that some of that section is
     not sent by anything yet. That is deliberate: hiding a row the server
     returns is how the five became untoggleable in the first place. */
]
...
export const settings = {
  ...
  consent: {
    ...
    async history({ signal, ...q } = {}) { return page(await http.get('/api/v1/settings/consent', q, { signal })) },   // Spring Page, occurredAt DESC
  ...
  blocks: {
    async list({ signal, ...q } = {}) { return page(await http.get('/api/v1/blocks', q, { signal }), userFrom) },
  ...
  notifications: {
    matrix() { return http.get('/api/v1/settings/notifications') },                // {TYPE:{PUSH:bool,…}} — all 44 rows
  ...
  safety: {
     ...
     *  MESSAGE targets: chat message ids are Snowflake STRINGS, not UUIDs.
     *  The wire's `targetId` is a UUID field that rejects them at binding —
     *  they go in `targetRef` instead, and the backend requires exactly one
     *  of id/ref (SubmitReportRequest / ReportService.submit). Routed here so
     *  callers can keep passing the id in `targetId` for every target type.
     *  @param {{targetType: string, targetId?: string|null, targetRef?: string|number|null, reason: string, details?: string|null}} req */
    report({ targetType, targetId, targetRef, reason, details }) {
      const ref = targetRef ?? (targetType === 'MESSAGE' ? targetId : undefined)
      return http.post('/api/v1/safety/reports', {
        targetType,
        targetId: ref != null ? undefined : targetId,
        targetRef: ref != null ? String(ref) : undefined,
        reason,
        details: details || undefined,
      })
    },
    async myReports({ signal, ...q } = {}) { return page(await http.get('/api/v1/safety/reports', q, { signal })) },   // Page<ReportResponse>
