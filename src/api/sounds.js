...
export const sounds = {
  ...
  /* DEPRECATED on the wire: sounds are admin-curated now — POST /api/v1/sounds
     is ADMIN/MODERATOR only (regular users get 403, no PENDING_REVIEW path)
     and answers with a Deprecation header pointing at POST /api/v1/admin/sounds. */
  upload(req)                   { return http.post('/api/v1/sounds', req) },
}
