/* =========================================================
   ...
   WHO USES IT
     ...
     'ika:signed-out'    AuthContext signOut() → per-account caches reset
                         (VOLUNTARY logout — auth-expired only covers the
                         involuntary path, and a second account must not
                         inherit the first's cached relationship edges)
     'ika:prefs'         theme/prefs.ts        → chatPrefs / mediaTier invalidate

   The web also bussed 'ika:compose' here. On native the composer is a
   route, so ComposeFab and every other entry point push '/compose'
   directly — there is no event, and no subscriber waiting for one.
...
export const SIGNED_OUT = 'ika:signed-out'
