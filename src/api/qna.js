...
export const qna = {
  /* ---- feeds / lifecycle ---- */
  async feed({ cursor, limit = 20, signal } = {}) {
    const page = await http.get('/api/v1/questions/feed/cursor', { cursor, limit }, { signal })
    return { items: (page?.items || []).map(questionFrom), nextCursor: page?.nextCursor ?? null, hasMore: !!page?.hasMore }
  },
  async list({ page = 0, size = 20 } = {}) {
    const res = await http.get('/api/v1/questions', { page, size })   // params are page/size only — ordering (newest-first) is server-owned
    return (res?.content || []).map(questionFrom)
  },
  async following({ page = 0, size = 20, signal } = {}) {         // §7.3
    const res = await http.get('/api/v1/questions/feed/following', { page, size }, { signal })
    return (res?.content || []).map(questionFrom)
  },
  async mine({ page = 0, size = 20, signal } = {}) {              // §6.5
    const res = await http.get('/api/v1/questions/me', { page, size }, { signal })
    return (res?.content || []).map(questionFrom)
  },
  ...
  /* ---- answers & reanswers ---- */
  async answers(questionId, { page = 0, size = 20, signal } = {}) {
    const res = await http.get(`/api/v1/questions/${questionId}/answers`, { page, size }, { signal })
  ...
  async addAttachment(questionId, answerId, formData, query, opts) {
    /* uploadX (RN XHR) when the caller wants progress/cancel. */
    const path = `/api/v1/questions/${questionId}/answers/${answerId}/attachments`
    return attachmentFrom(await (opts?.onProgress || opts?.signal
      ? http.uploadX(path, formData, { query, ...opts })
      : http.upload(path, formData, { query })))
  },
  ...
  async mySaved({ page = 0, size = 20, signal } = {}) {
    const res = await http.get('/api/v1/questions/me/saved', { page, size }, { signal })
    return (res?.content || res || []).map(questionFrom)
  },
  async mySavedCollection(name, { page = 0, size = 20, signal } = {}) {
    const res = await http.get('/api/v1/questions/me/saved/collection', { name, page, size }, { signal })
    return (res?.content || res || []).map(questionFrom)
  },
