...
function makeAnswer(db, questionId, req, media, voice, prefix = 'a-new') {
  ...
  const row = stampAuthor(db, {
    ...
    // The client sends the length it measured while recording; only fall back
    // to a stand-in when an older client omitted it.
    voiceDurationSeconds: voice ? (Number(req.voiceDurationSeconds) || 12) : null,
  })
